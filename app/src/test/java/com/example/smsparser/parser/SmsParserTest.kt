...
import com.example.smsparser.parser.config.BankConfig
...
class SmsParserTest {
    ...
    fun `test UPI exclusion`() {
        val sms =
            "Rs.99 debited from A/c XX4521 via UPI on 14-04-26. UPI Ref: 240478234511 to NETFLIX-MONTHLY."
    ...
    fun `test fintech co-branded issuer attribution`() {
        ...
        assertEquals(
            "9907",
            result.transaction?.cardLastFour
        )
    }
    ...
    fun `test foreign-currency transaction`() {
        val sms =
            "USD 49.99 spent on your Axis Bank Credit Card XX9876 at NETFLIX.COM/US on 13-APR-26. Foreign currency markup of 3.5% will be applied."
    ...
    @Test
    fun `test custom bank configuration`() {
        ...
        val customBankResolver = BankResolver(customConfig)

        val parser = SmsParser(
            bankResolver = customBankResolver
        )

        val sms =
            "INR 500.00 spent on New Bank Credit Card XX1234 at AMAZON on 15-04-2026."

        val result = parser.parse(sms, 9)
